...
* **Vector Memory Singleton:** In [`conftest.py`](../tests/conftest.py), the `vector_memory` ChromaDB client is swapped with a lightweight dictionary mock. This ensures the `get_hybrid_context` utility can be tested in milliseconds without requiring a live Ollama embedding service or a running database.
...
> **💡 Note on Dataset Prompts & Local Execution:** You will notice that the prompts within these datasets explicitly append the `/standard` command. This is a deliberate design choice for running evaluations. Relying on the Base Tier (Tier 1) for repeated, high-volume testing requires running heavy models locally, which can overwhelm host machine resources. Conversely, forcing the Frontier Tier (Tier 3) for iterative testing incurs unnecessary API costs.
